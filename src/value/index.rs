...
/// A type that can be used to index into a `serde_yaml_neo::Value`. See the `get`
    ...
    I: Index,
{
    ...
    /// Index into a `serde_yaml_neo::Value` using the syntax `value[0]` or
    /// `value["k"]`.
    ///
    ...
    /// let data: serde_yaml_neo::Value = serde_yaml_neo::from_str(r#"{ x: { y: [z, zz] } }"#)?;
    ///
    /// assert_eq!(data["x"]["y"], serde_yaml_neo::from_str::<Value>(r#"["z", "zz"]"#).unwrap());
    /// assert_eq!(data["x"]["y"][0], serde_yaml_neo::from_str::<Value>(r#""z""#).unwrap());
    ///
    /// assert_eq!(data["a"], serde_yaml_neo::from_str::<Value>(r#"null"#).unwrap()); // returns null for undefined values
    /// assert_eq!(data["a"]["b"], serde_yaml_neo::from_str::<Value>(r#"null"#).unwrap()); // does not panic
    /// # Ok(())
    /// # }
    /// ```
...
where
    I: Index,
{
    /// Write into a `serde_yaml_neo::Value` using the syntax `value[0] = ...` or
    /// `value["k"] = ...`.
    ///
    ...
    /// ```
    /// # fn main() -> serde_yaml_neo::Result<()> {
    /// let mut data: serde_yaml_neo::Value = serde_yaml_neo::from_str(r#"{x: 0}"#)?;
    ///
    /// // replace an existing key
    /// data["x"] = serde_yaml_neo::from_str(r#"1"#)?;
    ///
    /// // insert a new key
    /// data["y"] = serde_yaml_neo::from_str(r#"[false, false, false]"#)?;
    ///
    /// // replace a value in a sequence
    /// data["y"][0] = serde_yaml_neo::from_str(r#"true"#)?;
    ///
    /// // inserted a deeply nested key
    /// data["a"]["b"]["c"]["d"] = serde_yaml_neo::from_str(r#"true"#)?;
    ///
    /// println!("{:?}", data);
    /// # Ok(())
    /// # }
